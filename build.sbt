...
lazy val `sbt-github` = project
  ...
  .settings(libraryDependencies += "org.scalameta" %% "munit" % "0.7.9" % Test)
  .settings(libraryDependencies += "org.http4s" %% "http4s-dsl" % "0.21.5" % Test)
  .settings(libraryDependencies += "org.http4s" %% "http4s-blaze-server" % "0.21.5" % Test)
  .configs(CompileOnly)
  .settings(silencer)
